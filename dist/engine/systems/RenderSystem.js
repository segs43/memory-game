...
export class RenderSystem {
    ...
    render(player, npcs, particles, camera, corruption, glitch, elapsed) {
        const { canvas, ctx } = this;
        const W = canvas.width;
        const H = canvas.height;
        // Update motion trail
        this.playerTrail.push({ x: player.position.x, y: player.position.y, age: 0 });
        if (this.playerTrail.length > this.TRAIL_LENGTH)
    ...
    // ── Floor ──────────────────────────────────────────────────────────────────
    drawFloor(ctx, elapsed, corruption) {
    ...
    drawNPCGlowPools(ctx, npcs, elapsed) {
    ...
    drawNPCActive(ctx, x, y, alert, elapsed) {
        ...
        ctx.beginPath();
        ctx.ellipse(x - 5, y - 4, 5, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(x + 5, y - 4, 5, 4, 0, 0, Math.PI * 2);
        ctx.fill();
    ...
    // ── Player ────────────────────────────────────────────────────────────────
    drawPlayerTrail(ctx, corruptionLevel) {
    ...
    drawPlayer(ctx, player, elapsed, corruptionLevel) {
    ...
    // ── Vignette ──────────────────────────────────────────────────────────────
    drawVignette(ctx, player, camera, corruption, W, H) {
        ...
        // Inner bright zone
        const gradient = ctx.createRadialGradient(screenPos.x, screenPos.y, radiusPx * 0.15, screenPos.x, screenPos.y, radiusPx * 1.3);
    ...
    // ── Interaction hint ──────────────────────────────────────────────────────
    drawInteractionHint(ctx, player, camera, W, H, elapsed) {
