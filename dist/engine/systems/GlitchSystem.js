...
export function triggerGlitch(_state, intensity) {
    const blocks = Array.from({ length: Math.floor(2 + intensity * 6) }, () => ({
        y: Math.random() * 600,
...
export function updateGlitch(state, dt) {
    if (!state.active)
        return state;
...
export function renderGlitch(ctx, state, canvasW, canvasH) {
    if (!state.active)
        return;
    ...
    if (t > 0.6) {
        ...
        const gradient = ctx.createRadialGradient(canvasW / 2, canvasH / 2, canvasH * 0.2, canvasW / 2, canvasH / 2, canvasH * 0.8);
...
export function renderChromaticAberration(ctx, imageSrc, shift) {
    if (shift < 1)
        return;
    ctx.save();
