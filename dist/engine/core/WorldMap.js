...
function rectOverlapsCircle(wall, cx, cy, r) {
...
export function collidesWithWalls(walls, position, radius) {
    for (const wall of walls) {
        if (rectOverlapsCircle(wall, position.x, position.y, radius))
            return true;
    }
    return false;
}
export function resolveWallSlide(walls, from, to, radius) {
    // Try full move
    if (!collidesWithWalls(walls, to, radius))
        return to;
    // Try X only
    const slideX = { x: to.x, y: from.y };
    if (!collidesWithWalls(walls, slideX, radius))
        return slideX;
    // Try Y only
    const slideY = { x: from.x, y: to.y };
    if (!collidesWithWalls(walls, slideY, radius))
        return slideY;
    return from;
}
