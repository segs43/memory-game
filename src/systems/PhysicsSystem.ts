...
import type { GameEntity, WallRect, Vec2 } from '@core/types.js';
...
export class PhysicsSystem {
  ...
  isBlocked(x: number, y: number, r: number): boolean {
    for (const w of this.walls) {
      if (circleVsRect(x, y, r, w.x, w.y, w.w, w.h)) return true;
    }
    return false;
  }
   ...
   */
  distance(a: GameEntity, b: GameEntity): number {
   ...
   */
  separateEntities(entities: GameEntity[]): void {
