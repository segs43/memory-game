...
import type { PlayerEntity, NpcEntity, WallRect } from '@core/types.js';
import { CONFIG } from '@core/GameConfig.js';
import type { EffectSystem } from './EffectSystem.js';
...
export class RenderSystem {
  ...
  // NPC graphics map
  private npcGraphics = new Map<string, Graphics>();
  ...
  async init(walls: WallRect[]): Promise<void> {
  ...
  private buildWalls(walls: WallRect[]): void {
  ...
  private buildPlayerGfx(): Graphics {
  ...
  private buildNpcGfx(color: number): Graphics {
    const g = new Graphics();
    this.rebuildNpcGfx(g, color, false);
    return g;
  }

  private rebuildNpcGfx(g: Graphics, color: number, stolen: boolean): void {
    g.clear();
    if (stolen) {
  ...
  private drawVignette(px: number, py: number, radius: number): void {
  ...
  private drawNearbyHint(
    ...
    if (!npc || npc.memory.stolen) return;
  ...
  private postCtx: CanvasRenderingContext2D | null = null;

  private getPostCtx(): CanvasRenderingContext2D | null {
    if (this.postCtx) return this.postCtx;
  ...
  private renderPostFX(chromatic: number, scanlines: number): void {
    const ctx2 = this.getPostCtx();
    if (!ctx2 || !this.postCanvas) return;
