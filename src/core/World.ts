...
import type { PlayerEntity, NpcEntity } from './types.js';
...
export class World {
  ...
  async init(): Promise<void> {
    ...
    this.loop = new GameLoop(
      (dt) => this.update(dt),
      (interp) => this.renderFrame(interp),
      { fixedStep: 1 / 60, maxDelta: 0.1 }
    );
  ...
  private update(dt: number): void {
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (state.invertControls) { ndx = -ndx; ndy = -ndy; }
  ...
  // ── Render frame ───────────────────────────────────────────────
  private renderFrame(interp: number): void {
    const state = getState();
