...
import {
  ...
  updateGlitch,
  type GlitchState,
} from '@engine/systems/GlitchSystem';
import {
  applyMemorySteal,
  updateCorruptionTimers,
} from '@engine/systems/CorruptionSystem';
...
export class GameController {
  ...
  private glitch: GlitchState;
  private elapsed = 0;
  private glitchAccumulator = 0;

  // Random glitch interval tracking
  private nextGlitchIn = 3;
  ...
  private update(dt: number, elapsed: number): void {
    ...
    for (const npc of this.npcs) {
      if (npc.state !== 'stolen') this.particles.ambient(npc.position);
    }
  ...
  private getNearbyNPCIndex(): number {
  ...
  private stealMemory(index: number): void {
  ...
  private checkEndCondition(count: number): void {
  ...
  private render(): void {
    const store = useGameStore.getState();
    if (store.phase === 'menu' || store.phase === 'ending') return;

    this.renderer.render(
      this.player,
      this.npcs,
      this.particles.getActive(),
      this.camera,
      this.corruption,
      this.glitch,
      this.elapsed,
    ...
    if (store.npcNearby && store.phase === 'playing') {
      this.renderer.drawInteractionHint(
